<!DOCTYPE html>
<html>
<head>
	<title>Membuat CRUD dengan CodeIgniter</title>
</head>
<body>
	<center>
		<h1>Membuat CRUD dengan CodeIgniter</h1>
		<h3>Edit Data</h3>
	</center>
	<?php foreach($user as $u){ ?>
	<form action="<?php echo site_url('Main/proses_check_out')?>" method="post">
		<table style="margin:20px auto;">
			<tr>
				<td>Nama</td>
				<td>
					<input type="hidden" name="id_bukutamu" value="<?php echo $u->id_bukutamu ?>">
					<input type="text" name="nama" value="<?php echo $u->nama ?>" readonly="">
				</td>
			</tr>
			<tr>
				<td>Alamat</td>
				<td><input type="text" name="instansi" value="<?php echo $u->instansi ?>" readonly=""></td>
			</tr>
			<tr>
				<td>Pekerjaan</td>
				<td><input type="text" name="kegiatan" value="<?php echo $u->kegiatan ?>" readonly=""></td>
			</tr>
			<tr>
				<td></td>
				<td><input type="submit" value="Check Out"></td>
			</tr>
		</table>
	</form>	
	<?php } ?>
</body>
</html>